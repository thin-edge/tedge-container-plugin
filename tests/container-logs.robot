...
Get Container Logs with only last N lines
    # Run dummy container then exit
    DeviceLibrary.Execute Command    cmd=tedge-container engine docker run --name app11 httpd:2.4.61-alpine sh -c 'echo hello inside container stdout; sleep 1; echo hello inside container stderr >&2;'
